...
import org.jetbrains.annotations.Nullable; // <--- ДОБАВИЛИ ЭТОТ ИМПОРТ
...
public class WindmillRotorBlockEntity extends BlockEntity {
    ...
    // ВАЖНО: Эта переменная решает рассинхрон с сервером
    ...
    public static void tick(Level level, BlockPos pos, BlockState state, WindmillRotorBlockEntity entity) {
        entity.isSpinning = true; // Подтверждаем серверу, что работаем
